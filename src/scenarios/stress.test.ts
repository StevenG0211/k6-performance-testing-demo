...
import { stressTestOptions } from '../config/test-options';
 ...
 * Uses stressTestOptions configuration from test-options.ts
 */
...
export function setup(): Record<string, unknown> {
    logTestInfo('Stress test setup - initializing test data');
    return {
...
export function teardown(data: Record<string, unknown>): void {
    logTestInfo('Stress test teardown', data);
}
...
export function handleSummary(data: unknown): Record<string, string> {
    const reportName = __ENV.REPORT_NAME || 'stress';
