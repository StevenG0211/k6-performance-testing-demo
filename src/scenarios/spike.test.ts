...
import { spikeTestOptions } from '../config/test-options';
 ...
 * Uses spikeTestOptions configuration from test-options.ts
 */
...
export default function (): void {
    ...
    const getChecks = check(getResponse, {
        ...
        'GET response time < 3000ms': (r) => r.timings.duration < 3000,
...
export function setup(): Record<string, unknown> {
    logTestInfo('Spike test setup - initializing test data');
    return {
...
export function teardown(data: Record<string, unknown>): void {
    logTestInfo('Spike test teardown', data);
}
...
export function handleSummary(data: unknown): Record<string, string> {
    const reportName = __ENV.REPORT_NAME || 'spike';
